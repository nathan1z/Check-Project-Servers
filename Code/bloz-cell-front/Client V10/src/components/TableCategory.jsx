...
class TableCategory extends React.Component {
	...
	//Modal insert
	modalInsert = () => {
		const nextId = this.state.data.length
			? Math.max(...this.state.data.map((categoria) => categoria.idCategoria)) +
			  1
			: 1;

		this.setState({
			modalInsert: !this.state.modalInsert,
			form: { ...this.state.form, idCategoria: nextId },
		});
	};
